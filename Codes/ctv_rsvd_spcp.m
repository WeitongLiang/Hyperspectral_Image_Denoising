...
function [ output_image,E] =ctv_rsvd_spcp(oriData3_noise,sigma,weight)
tic
...
while iter<maxIter
    iter          = iter + 1;   
    %% -Updata X1,X2,X3
    [u,s,v] = rSVD(reshape(diff_x(X,sizeD),[M*N,p])+M2/mu);
    X1      = u*softthre(s,1/mu)*v';
    [u,s,v] = rSVD(reshape(diff_y(X,sizeD),[M*N,p])+M3/mu);
    X2      = u*softthre(s,1/mu)*v';
    [u,s,v] = rSVD(reshape(diff_z(X,sizeD),[M*N,p])+M4/mu);
