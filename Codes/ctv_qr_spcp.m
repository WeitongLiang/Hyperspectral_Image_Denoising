function [ output_image,E] =ctv_qr_spcp(oriData3_noise,sigma,weight)
tic
oriData3_noise=im2double(oriData3_noise);
tol     = 1e-6;
maxIter = 400;
rho     = 1.25;
% max_mu  = 1e6;
...
while iter<maxIter
    iter          = iter + 1;   
    %% -Updata X1,X2,X3
    [u,s,v] = QR_SVD(reshape(diff_x(X,sizeD),[M*N,p])+M2/mu);
    X1      = u*softthre(s,1/mu)*v';
    [u,s,v] = QR_SVD(reshape(diff_y(X,sizeD),[M*N,p])+M3/mu);
    X2      = u*softthre(s,1/mu)*v';
    [u,s,v] = QR_SVD(reshape(diff_z(X,sizeD),[M*N,p])+M4/mu);
